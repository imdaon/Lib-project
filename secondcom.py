...
def rent_book(): #대여
    ...
    while(1):        
        ...
        if select2 == '2': #대여
            ...
            bookselect=int(input('대여할 책의 번호를 입력해주세요: '))
            for i in bucket:
                if bookselect == i[0]:
                    ...
                    if find is None:
                        ...
                        print(find[2])
                        print(find)
                        cur.execute("INSERT INTO rentlist VALUES(?,?,?,?,date('now'))",(find[0],find[1],find[2],userid))
                        conn.commit()
                        conn.close()
...
def return_book():
    ...
    if turnback=='1':
        ...
        if find is None:
            ...
            conn.commit()
            cur.execute("INSERT INTO returnlist VALUES(?,?,?,date('now'))",(find[0],find[1],find[2]))
            conn.commit()
            conn.close()
            os.system("clear")
            time.sleep(1)
